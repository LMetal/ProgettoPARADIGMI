...
public class Agende {
    ...
    public static void createAgenda(String nome) throws AgendeException {
        Agenda newAgenda = new Agenda(nome);
        if(isNameUnico(newAgenda)){
            agende.add(newAgenda);
        } else {
            throw new AgendeException("Nome gia' in uso");
        }
    }

    private static boolean isNameUnico(Agenda newAgenda) {
        return agende.stream().noneMatch(a -> a.compareNome(newAgenda));
    }
